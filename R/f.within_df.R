...
f.within_df <- function(data, id, col, na.rm = TRUE){
  ...
  if(str_detect(id_as_string, pattern = "\\{")){
    id_as_string <- str_trim(str_remove_all(string = stra, pattern = "\\{|\\}|"))
  } else {id_as_string}
  ...
  c <- b %>%
    mutate(dev = value - pull_id_mean + pull_o_mean) # following Stata's way to compute within sd
  ...
  within_df <- c %>%
    ...
    rename_at(.vars = col, .funs = function(x){paste0("within_sd.",x)})
  return(within_df)
}
